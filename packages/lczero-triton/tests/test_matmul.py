...
@pytest.mark.parametrize(
    ("m", "n", "k", "architecture"),
    [(0, 1, 1, 80), (1, 0, 1, 80), (1, 1, 0, 80), (1, 1, 1, 0)],
)
def test_specialization_rejects_invalid_static_values(
    ...
    """Static dimensions and architecture must all be positive."""
    with pytest.raises(ValueError, match="positive"):
        MatmulSpecialization(m, n, k, architecture)


def test_compilation_rejects_a_different_active_architecture() -> None:
    """Autotuning cannot silently produce a CUBIN for the wrong target."""
    specialization = MatmulSpecialization(1, 1, 1, _architecture() + 1)

    with pytest.raises(ValueError, match="active device"):
        compile_matmul(specialization)
...
def test_graph_call_rejects_output_aliasing_before_compilation() -> None:
    ...
    with pytest.raises(ValueError, match="cannot alias"):
        matmul(
            ...
            buffer,
            buffer,
            MatmulSpecialization(1, 1, 1, _architecture()),
        )
